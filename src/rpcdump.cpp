...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "importprivkey <fluttercoinprivkey> [label]\n"
    ...
    if (fWalletUnlockStakingOnly) // fluttercoin: no importprivkey in mint-only mode
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <fluttercoinaddress>\n"
            "Reveals the private key corresponding to <fluttercoinaddress>.");
    ...
    if (fWalletUnlockStakingOnly) // fluttercoin: no dumpprivkey in mint-only mode
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Wallet is unlocked for minting only.");
    CKeyID keyID;
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by FlutterCoin %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
